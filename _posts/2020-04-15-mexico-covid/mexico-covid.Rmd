---
title: "Estimación del impacto de las medidas de mitigación por COVID-19 utilizando un modelo jerárquico Bayesiano (Actualización Mayo 6)"
author:
...
output:
  distill::distill_article:
    ...
    toc: yes
    toc_depth: 3
repository_url: https://github.com/juandiazmart/blog/tree/master/_posts/2020-04-15-mexico-covid
---
...
un_dates <- unique(covariates_country_long$value)
  ...
  p1 <- ggplot(data_country) +
    ...
    xlab("") +
    ylab("Número acumulado de infectados con SARS-Cov-2") +
...
1. Cancelación de eventos públicos - Se asume como el día (Marzo 17) en que el gobierno de la CDMX decidió cancelar todos los eventos públicos en la [ciudad](https://www.eluniversal.com.mx/metropoli/cdmx/coronavirus-gobierno-de-cdmx-suspende-eventos-masivos-por-covid-19).

2. Cierre de escuelas - Se asume como el día (Marzo 21) en que la UNAM suspendió clases [clases](https://heraldodemexico.com.mx/pais/unam-suspende-clases-por-coronavirus-covid-19/). 

3. Exhortación al distanciamiento social - Inicio de Susana [Distancia](https://www.unotv.com/noticias/portal/nacional/detalle/coronavirus-en-mexico-anuncian-jornada-nacional-de-sana-distancia-491479/) (Marzo 23).

4. Exentensión de la Jornada Nacional de Sana distancia - 16 de abril.
...
ggplotly(p1) %>% 
 layout(annotations = 
 list(x = 1, y =0, text ="Juan Pablo Díaz @juandiazmart.", 
...
Al inicio de la epidemia en México, el número efectivo de reproducción $R_t$  (ver Figura \@ref(fig:infect)) era de `r formatC(head(data_country[,"rt"],1), format="f", big.mark=",", digits=1)` (95% CrI: `r paste(formatC(head(data_country[,"rt_min"],1), format="f", big.mark=",", digits=1),"-",formatC(head(data_country[,"rt_max"],1), format="f", big.mark=",", digits=1))`). Después de que el gobierno extendió "Susana Distancia" el 16 de abril, éste descendió a `r formatC(tail(data_country[,"rt"],1), format="f", big.mark=",", digits=1)` (95% CrI: `r paste(formatC(tail(data_country[,"rt_min"],1), format="f", big.mark=",", digits=1),"-",formatC(tail(data_country[,"rt_max"],1), format="f", big.mark=",", digits=1))`).

```{r repro,message=FALSE, warning=FALSE, tidy=TRUE, fig.height=8,fig.width=12,fig.cap=paste("Efecto de las intervenciones en el número efectivo de reproducción")}
p3+labs(title="Numero efectivo de reproducción",
  caption = "Juan Pablo Díaz @juandiazmart / Datos: Informe diario S. Salud (https://github.com/mariorz).",
        subtitle = paste("Datos hasta", format(max(data_covid$date), "%b %e, %Y"))) + 
   theme(plot.title = element_text(size = rel(2), face = "bold"),
         plot.subtitle = element_text(size = rel(1.5)),
         axis.text.y = element_text(size = rel(2)),
...
1. Reparametrizar algunos parámetros para incluir otras covariables.
2. Predictive checks.
